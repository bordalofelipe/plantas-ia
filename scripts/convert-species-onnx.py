...
class PlantSpeciesModelResNet50(torch.nn.Module):
    def __init__(self, num_classes):
        super().__init__()
...
model = torch.load('../models/plant-species.pth', weights_only = False)
...
torch.onnx.export(model, dummy_input, "../docs/plant-species.onnx", 
    input_names=['input.1'],
    output_names=['output']
)
